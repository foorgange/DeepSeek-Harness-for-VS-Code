...
async function main() {
  ...
  try {
    ...
    // 端口关闭 ≠ exit 回调已经跑过。Windows 上 shell:true 多包了一层 cmd.exe:里层的 node
    // 先死、端口先关,cmd.exe 慢几十毫秒才退出,exit 回调也就慢那一下(实测 ≤110ms)。
    // 所以这里等**状态本身**收敛,而不是拿端口当代理 —— 后者约 1/3 概率把正确实现误报成失败。
    const settled = await waitFor(() => Promise.resolve(mgr.status.up === false), 5000);
    check("崩溃后 status.up === false", settled === true, JSON.stringify(mgr.status));
